...
if(isset($_POST["prev"]))
{
	...
	$query ="SELECT * FROM $_TABLE_ACC INNER JOIN $_TABLE_NAME2 ON $_TABLE_ACC.productId = $_TABLE_NAME2.productId and $_TABLE_NAME2.CATEGORY='Accessory' and
		$_TABLE_ACC.type='Case' LIMIT ".(($currentPage-1)*$numPerPage).",$numPerPage";
}
else
{
	if(isset($_POST["next"]))
	...
	else
	{
		...
		$currentPage = 1;
		$query ="SELECT * FROM $_TABLE_ACC INNER JOIN $_TABLE_NAME2 ON $_TABLE_ACC.productId = $_TABLE_NAME2.productId and $_TABLE_NAME2.CATEGORY='Accessory' and
		$_TABLE_ACC.type='Case' LIMIT 0,$numPerPage";
	}
}
...
if(isset($_POST["prev"]))
{
	...
	?>
   	  <div class="center_title_barlc">"Cases" <?php echo $numRecord;?> results found.</div>
   	  <?php
		for ($i = 0; $i < $num; $i++) {
