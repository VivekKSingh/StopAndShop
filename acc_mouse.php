  ...
  <body>
	...
	<div class="crumb_navigationlc">Navigation: 
	 <label><a href="index.php">Home</a></label> >
	  ...
	  <span class="currentlc"><?php echo $acc_type;?></span> 
	</div>    
	...
	<div class="left_contentlc">
	  ...
	  <ul class="left_menulc">
		...
		<li class="even"><a href="acc_mouse.php?acc_type=Hard Drive">Hard Disk</a></li>
	  </ul>
	</div> 
	<!-- end of left content -->
...
if(isset($_POST["prev"]))
{
	...
	$query ="SELECT * FROM $_TABLE_ACC INNER JOIN $_TABLE_NAME2 ON $_TABLE_ACC.productId = $_TABLE_NAME2.productId and $_TABLE_NAME2.CATEGORY='Accessory' and
		$_TABLE_ACC.type= '$acc_type' LIMIT ".(($currentPage-1)*$numPerPage).",$numPerPage";
}
else
{
	if(isset($_POST["next"]))
	...
	else
	{
		...
		$currentPage = 1;
		$query ="SELECT * FROM $_TABLE_ACC INNER JOIN $_TABLE_NAME2 ON $_TABLE_ACC.productId = $_TABLE_NAME2.productId and $_TABLE_NAME2.CATEGORY='Accessory' and
		$_TABLE_ACC.type='$acc_type' LIMIT 0,$numPerPage";
	}
}
...
if(isset($_POST["prev"]))
{
	...
	?>
   	  <div class="center_title_barlc"> <?php echo $acc_type."   ".$numRecord;?> results found.</div>
   	  <?php
		for ($i = 0; $i < $num; $i++) {
	  ...
	  <div class="prod_box_big"> 
		...
		<a href="details_acc.php?prodid=<?php echo $prodid;?>" title="header=[Zoom] body=[&nbsp;] fade=[on]"><img src=<?php echo $imagepath;?> alt="" title="" border="0" /></a>
	      </div>
        	...
        	<div class="cart_titlelc">Shopping cart</div>
             ...
             <?php 
   				if(isset($_SESSION["custID"]))
   				{
   					...
   					$query = "select sum(price*quantity) as pricetotal,  sum(quantity) as numtotal from shoppingcart where customerID = '$customerID' ";
   					$result = mysql_query ($query);
   					$pricetotal = 0;
