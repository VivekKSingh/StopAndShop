...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  $size= $_POST["size"];
   ...
   if(isset ($_POST["HDD"]))
	$hdd=$_POST["HDD"];
  $query = "SELECT * FROM $_TABLE_NAME INNER JOIN $_TABLE_NAME2 ON $_TABLE_NAME.productId = $_TABLE_NAME2.productId";
  //echo $query;
  if(!empty($laptop))
  {
  $N = count($laptop);
   for($i=0; $i < $N; $i++)
	...
	if (count($cond)) {
    $query .= " and BRAND IN (" . implode(" , ", $cond). ")";
	}
	...
	{   $pos=1;$value_ran=0;
		...
		$query .= " and WEIGHT IN (" . implode(" , ", $wLaptop). ")";
	...
	if(!empty($size))
	{
	   ...
	   for($i=0; $i < $N; $i++)
		...
		if (count($sizeLaptop)) {
		$query .= " and SIZE IN (" . implode(" , ", $sizeLaptop). ")";
		}
   ...
   if(!empty($cpu))
	{
	   ...
	   for($i=0; $i < $N; $i++)
		...
		if (count($cpuLaptop)) {
		$query .= " and processor IN (" . implode(" , ", $cpuLaptop). ")";
		}
	...
	if(!empty($color))
	{
	   ...
	   for($i=0; $i < $N; $i++)
		...
		if (count($colorLaptop)) {
		$query .= " and COLOR IN (" . implode(" , ", $colorLaptop). ")";
		}
	...
	if(!empty($price))
	{
	   ...
	   for($i=0; $i < $N; $i++)
		...
		if (count($priceLaptop)) {
		$query .= " and PRICE BETWEEN ".$priceLaptop[0];
		}
	}
	  ...
	  if(!empty($hdd))
	{
	   ...
	   for($i=0; $i < $N; $i++)
		...
		if (count($hddLaptop)) {
		$query .= " and harddisk IN (" . implode(" , ", $hddLaptop). ")";
		}
	...
	if(isset($_POST["prev"]))
	{
		...
		//Build DQL
		$query .=" LIMIT ".(($currentPage-1)*$numPerPage).",$numPerPage";
	}
	else
	{
		if(isset($_POST["next"]))
	...
	?>
	<div class="center_title_barlc"> <?php echo $numRecord;?> results found.</div>
   ...
   <!-- A row for paginations -->
      ...
      <div class="pagi_rowlc">
		...
		<?php 
		if (isset($_POST['laptop'])) { ?>
		        <input name="laptop[]"  type="hidden" value=<?php $_POST["laptop"]?> /> <?php }?> <?php 
		if (isset($_POST['weight'])) { ?>
		        <input name="weight[]"  type="hidden" value=<?php  $_POST["weight"];?> /> <?php }?> <?php 
		if (isset($_POST['size'])) { ?>
		        <input name="size[]"  type="hidden" value=<?php $_POST["size"];?> /> <?php }?> <?php 
		if (isset($_POST['cpu'])) { ?>
		        <input name="cpu[]"  type="hidden" value=<?php $_POST["cpu"];?> /> <?php }?> <?php 
		
		if (isset($_POST['price'])) { ?>
		        <input name="price[]"  type="hidden" value=<?php $_POST["price"];?> /> <?php }?> <?php 
		if (isset($_POST['color'])) { ?>
		        <input name="color[]"  type="hidden" value=<?php $_POST["color"];?> /> <?php }?> 
   
		<input name="temp" type="hidden" value=<?php echo $tempquery;?> />
